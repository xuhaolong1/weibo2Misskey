...
class Weibo(object):
    def __init__(self, config):
        ...
        self.user_id_list = [user['id'] for user in config['user_list']]
...
#user_id_list
    ...
    def is_pinned_weibo(self, info):
        ...
        if 'mblog' in info:
            weibo_info = info['mblog']
            if 'title' in weibo_info and weibo_info['title']['text'] == '置顶':
                return True
        return False
            ...
            if js['ok']:
                ...
                for w in weibos:
                    if w['card_type'] == 9:
                        if 'mblog' in w:
                            wb = w['mblog']
                            if not self.is_pinned_weibo(wb) and 'retweeted_status' not in wb:
                                wb = self.parse_weibo(wb)
